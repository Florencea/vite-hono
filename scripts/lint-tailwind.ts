...
async function main() {
  ...
  const cssPath = path.resolve(projectRoot, "src/client/global.css");
  ...
  const clientDir = path.resolve(projectRoot, "src/client");
  const files = collectSourceFiles(clientDir);
